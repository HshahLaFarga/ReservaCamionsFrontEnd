...
export class LockMuelleAddUpdateService {
  ...
  addLockMuelle(lockMuelle: Booking): Observable<any> {
    ...
    return this.http.post(`${environment.apiBaseUrl}/reservas/bloqueos`,lockMuelle);
  }

  updateLockMuelle(lockMuelle: Booking, lockMuelle_id: number): Observable<any> {
    return this.http.put(`${environment.apiBaseUrl}/reservas/bloqueos/${lockMuelle_id}`,lockMuelle);
  }
}
